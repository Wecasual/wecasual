...
function upcomingGames(scheduleRepo, req, res){
  scheduleRepo.getOneSchedule(req.user.team.id, function(err, result){
    if(err){
      res.send({
        success: false,
...
function updateAttendance(scheduleRepo, req, res){
  scheduleRepo.updateAttendance(req.body.gameid, req.body.userid, req.body.attendance, req.body.teamNum, function(err){
    if(err){
      res.send({
        success: false,
...
module.exports = (profilesRepo, scheduleRepo) => {
  return{
    profile: {
      route: '/profile',
    ...
    upcomingGames: {
      route: '/profile/upcomingGames',
      handler: upcomingGames.bind(null, scheduleRepo)
    },
    updateAttendance: {
      route: '/profile/updateAttendance',
      handler: updateAttendance.bind(null, scheduleRepo)
    }
  }
}
