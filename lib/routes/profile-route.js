function getAllUsers(profilesRepo, req, res){
  ...
  var select = "playerid, username, avatar, premium";//, avatar, registrationdate, wecasualpoints, team, totalgames, premium"
...
function getFriendReq(profilesRepo, req, res){
  profilesRepo.getFriendReq(req.body.playerid, function(err, friendReq){
    ...
    if(err) {
      ...
      profilesRepo.getSentFriendReq(req.body.playerid, function(err, sentFriendReq){
        if(err) {
          console.log(err);
          res.send({
            success: false,
...
function declineFriend(profilesRepo, req, res){
  profilesRepo.declineFriendReq(req.body.playerfrreqid, function(err){
    if(err) {
      console.log(err);
      res.send({
        success: false,
...
function getUser(profilesRepo, req, res){
...
module.exports = (profilesRepo) =>{
  return{
    ...
    getUser: {//Returns all user info for a specified user (including team info)
      route: '/profile/getUser',
      handler: getUser.bind(null, profilesRepo)
    }
