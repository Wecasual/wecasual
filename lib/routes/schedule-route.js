...
function scheduleGame(scheduleRepo, discordBot, req, res){
  ...
  scheduleRepo.scheduleGame(info, function(err, game){
    if(err){
      ...
      res.send({
        success: false,
        error: "Unable to schedule game"
      });
    }
    else{
      var gametime = toTimeZone(game.gametime, 'America/Toronto') + " EST";
      if(req.body.announce === 'true'){
        discordBot.postQuickLink([req.body.gameType, gametime, game.gameid, req.body.username]);
      }
      req.session.message = game.name + " scheduled for " + gametime;
      res.send({
        success: true,
        game: game,
...
function toTimeZone(time, zone) {
    ...
    return moment(time, format).tz(zone).format('YYYY/MM/DD hh:mmA');
}
...
function getSingleGame(scheduleRepo, req, res){
  scheduleRepo.getSingleGame(req.body.gameid, function(err, game){
    if(err){
      ...
      gametime: toTimeZone(prevPlayergame.gametime,'America/Toronto') + " EST",
      ...
      else if(prevPlayergame.gametime < new Date()){
        res.send({
          success: false,
