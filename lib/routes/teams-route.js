		...
		if(err){
			...
			for(var i = 0; i < 5; i++){
				if(roster["p" + (i+1)].id == playerid){
					roster["p" + (i+1)].id = undefined;
					roster["p" + (i+1)].avatar = undefined;
					roster["p" + (i+1)].displayName = undefined;
					break;
				}
			}
...
function addPlayer(teamsRepo, profilesRepo, req, res){
	// console.log(req.body);
	...
	teamsRepo.getTeam(teamid, function(err, result){
			if(err){
				...
				for(var i = 0; i < 5; i++){
					if(roster["p" + (i+1)].displayName === undefined){
						roster["p" + (i+1)].id = id;
						roster["p" + (i+1)].avatar = avatar;
						roster["p" + (i+1)].displayName = gf.addEscape(displayName);
						break;
					}
				}
				console.log(roster);
				teamsRepo.updateTeam([{field: "roster", value: "'" + [JSON.stringify(roster)].join("','") + "'"}], teamid, function(err){
					if(err){
						...
						profilesRepo.updateUser([{field: "team", value: "'" + [JSON.stringify(userTeam)].join("','") + "'"}], id, function(err){
							if(err){
								res.send({
									success: false,
									error: 'Error updating player team'
    ...
    getTeams: {
      ...
      handler: getTeam.bind(null, teamsRepo)
		...
		addPlayer: {
			...
			handler: addPlayer.bind(null, teamsRepo, profilesRepo)
		}
