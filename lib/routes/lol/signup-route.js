...
function submit(profilesRepo, req, res) {
  ...
  //   }
  // }
  profilesRepo.updateUser({"Summoner Name": req.body.name, "Status": "Registered", "Skill Level": req.body['skill-level'], "Registration Date": new Date(), "Id": req.user['Id']}, req.user['Id'], function(err, user){
    if(err){
      console.log(err);
    }
    else{
      req.user['Status'] = "Registered";
      req.user['Summoner Name'] = req.body.name;
