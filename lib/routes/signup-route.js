...
function updateAfterCharge(profilesRepo, req, res){
  ...
  profilesRepo.updateUser([{field: "email", value: req.body.stripeEmail}, {field: "paid", value: true}, {field: "playerrequests", value: JSON.stringify(playerRequestsJson)}, {field: "skilllevel", value: req.session.signupInfo['skill-level']}], req.user.id, function(err){
    if(err){
      console.log(err);
    }
    req.user.paid = true;
