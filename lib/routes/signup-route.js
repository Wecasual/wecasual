...
function submit(profilesRepo, req, res) {
  ...
  profilesRepo.updateUser({"Email": req.body.email, "Status": "Registered", [realm + "SkillLevel"]: req.body['skill-level'], "Registration Date": new Date(), "Id": req.user['Id'], [realm]: true}, req.user['Id'], function(err, user){
    if(err){
      console.log(err);
    }
    else{
...
function submitSkillLevel(profilesRepo, req, res) {
  var realm = req.session.realm;
  profilesRepo.updateUser({[realm + "SkillLevel"]: req.body['skill-level'], [realm]: true}, req.user['Id'], function(err, user){
    if(err){
      console.log(err);
    }
    else{
