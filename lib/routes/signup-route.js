...
function submit(profilesRepo, req, res) {
  ...
  //   }
  // }
  profilesRepo.updateUser({"Email": req.body.email, "Status": "Registered", "Skill Level": req.body['skill-level'], "Id": req.user['Id'], "Registration Date": new Date()}, req.user['Id'], function(err, user){
    if(err){
      console.log(err);
    }
    else{
