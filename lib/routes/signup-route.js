...
function payment(keyPublishable, profiles, req, res) {
  ...
  profiles.updateUser([{field: "email", value: req.session.signupInfo.email}, {field: "registered", value: true}, {field: "playerrequests", value: JSON.stringify(playerRequestsJson)}, {field: "skilllevel", value: req.session.signupInfo['skill-level']}], req.user.id, function(err){
    if(err){
      console.log(err);
    }
...
//
//   });
//   profiles.updateUser([{field: "email", value: req.body.stripeEmail}, {field: "registered", value: true}, {field: "playerrequests", value: JSON.stringify(playerRequestsJson)}, {field: "skilllevel", value: req.session.signupInfo['skill-level']}], req.user.id, function(err){
//     if(err){
//       console.log(err);
//     }
//     req.user.registered = true;
...
module.exports = (profiles, stripe, keyPublishable) =>{
  return{
    ...
    payment: {
      ...
      handler: payment.bind(null, keyPublishable, profiles)
    },
    submit: {
      route: '/signup/submit',
