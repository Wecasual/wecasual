  ...
  $.ajax({
    ...
    success: function(res){
      if(!res.success){
        ...
        var html = "";
        res.data.friendReq.forEach(function(frReq){
          ...
          <button id="req-' + frReq.playerfrreqid + '" type="button" class="btn btn-danger">&#10008;</button></div></div>';
          friendRequestid.push(frReq.reqid);
        });
        $('#friend-requests').html(html);
        html = "";
        res.data.sentFriendReq.forEach(function(frReq){
          var avatar = frReq.avatar;
          if(avatar.includes('null')){
            avatar='/images/avatar-default.png';
          }
          html += '<div class="row mb-1"><div class="col-md-10"><a class="roster-link" href="dota/players?playerid=' + frReq.playerid +
          '" target="_blank"><div class="card-player mb-1 p-1"><img height="20" width="20" class = "rounded-circle" src=' + avatar + ' alt"Avatar"> ' +
          frReq.username + '</div></a></div><div class="col-md-2"><button id="req-' + frReq.playerfrreqid + '" type="button" class="btn btn-danger">&#10008;</button></div></div>';
          friendRequestid.push(frReq.playerid);
        });
        $('#sent-friend-requests').html(html);
      }
    }
  });
  $.ajax({
...
$(document).on('click', '.btn-danger', function(){
  playerfrreqid = this.id.substring(4, this.id.length);
  $.ajax({
    type: 'POST',
    url: '/profile/declineFriend',
    contentType: 'application/json',
    data: JSON.stringify({
      playerfrreqid: playerfrreqid
    }),
    success: function(res){
      if(!res.success){
        alert(res.error);
